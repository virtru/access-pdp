...
// - that is, the aggregate result of comparing entity attributes to every data attribute.
...
//DataRuleResult represents the rule-level (or attribute-definition-level) decision for a specific entity -
//the result of comparing entity attributes to a single data attribute definition/rule (with potentially many values)
...
//ValueFailure indicates, for a given entity and data attribute, which data values
//(aka specific data AttributeInstance) the entity "failed" on.
//A single data attribute may be present in the data policy with one or more values.
//A value failure does not necessarily mean a rule failure, it is purely informational -
//there will be one value failure, per entity, per rule, per value the entity lacks -
//it is up to the rule itself to translate this into an overall failure or not.
...
func (pdp *AccessPDP) DetermineAccess(dataAttributes []attrs.AttributeInstance, entityAttributeSets map[string][]attrs.AttributeInstance, attributeDefinitions []attrs.AttributeDefinition, parentCtx ctx.Context) (map[string]*Decision, error) {
	...
	for canonicalName, distinctValues := range clusteredDataAttrs {
		...
		//If GroupBy is set, determine which entities (out of the set of entities and their respective attributes)
...
//AllOf the Data Attribute CanonicalName+Value pairs should be present in AllOf the Entity's Attribute sets
//Accepts
...
func (pdp *AccessPDP) AllOfRule(dataAttrsBySingleCanonicalName []attrs.AttributeInstance, entityAttributes map[string][]attrs.AttributeInstance, groupBy *attrs.AttributeInstance) map[string]DataRuleResult {
	...
	//Go through every entity's attribute set...
	for entityId, entityAttrs := range entityAttributes {
		var valueFailures []ValueFailure
		//Default to DENY
		entityPassed := false
		//Cluster entity attributes by canonical name...
		entityAttrCluster := attrs.ClusterByCanonicalName(entityAttrs)

		//For every unqiue data attribute value in this set of data attrs sharing the same canonical name...
		for _, dataAttrVal := range dataAttrsBySingleCanonicalName {
			...
			//See if
			// 1. there exists an entity attribute in the set of attributes
			// with the same canonical name as the data attribute in question
			// 2. It has the same VALUE as the data attribute in question
			found := findInstanceValueInCluster(&dataAttrVal, entityAttrCluster[dvCanonicalName])

			denialMsg := ""
			//If we did not find the data attribute canonical name + value in the entity attribute set,
			//then prepare a ValueFailure for that data attribute and value, for this entity
			if !found {
...
//AnyOf the Data Attribute CanonicalName+Value pairs can be present in AnyOf the Entity's Attribute sets
//Accepts
// - a set of data attribute instances with the same canonical name
// - a map of entity attribute instances keyed by entity ID
//Returns a map of DataRuleResults keyed by EntityID
func (pdp *AccessPDP) AnyOfRule(dataAttrsBySingleCanonicalName []attrs.AttributeInstance, entityAttributes map[string][]attrs.AttributeInstance, groupBy *attrs.AttributeInstance) map[string]DataRuleResult {
	...
	for entityId, entityAttrs := range entityAttributes {
		...
		//AnyOf - IF there were fewer value failures for this entity, for this attribute canonical name,
		//then there are distict data values, for this attribute canonical name, THEN this entity must
		//possess AT LEAST ONE of the values in its entity attribute cluster,
...
//Hierarchy rule compares the HIGHEST (that is, numerically lowest index) data value for a given attribute canonical name
//with the LOWEST (that is, numerically highest index) entity value for a given attribute canonical name.
//
//If multiple data values for a hierarchy attribute are present for the same canonical name, the highest will be chosen and
//the others ignored.
//
...
func (pdp *AccessPDP) HierarchyRule(dataAttrsBySingleCanonicalName []attrs.AttributeInstance, entityAttributes map[string][]attrs.AttributeInstance, groupBy *attrs.AttributeInstance, order []string) map[string]DataRuleResult {
	...
	for entityId, entityAttrs := range entityAttributes {
		...
		//Compare the (one or more) entity values for this canonical attribute to the (one) data value, and see which is "higher".
...
//the purpose of a GroupBy attribute is to indicate which entities should be included in a rule evaluation, and which
...
func entityRankGreaterThanOrEqualToDataRank(order []string, dataAttribute *attrs.AttributeInstance, entityAttributeCluster []attrs.AttributeInstance) bool {
	...
	// for a given data attribute canonical name,
	// we may have *several* entity values for a given entity attribute canonical name -
	...
	for _, entityAttribute := range entityAttributeCluster {
		//Ideally, the caller will have already ensured all the entity attributes we've been provided
		//have the same canonical name as the data attribute we're comparing against,
		// but if they haven't for some reason only consider matching entity attributes
...
//return the rank #/index of the singular attribute instance
